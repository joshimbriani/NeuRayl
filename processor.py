...
def getBattersData():
	with open('data/Batting.csv', 'rb') as csvfile:
		data = []
...
def getPitchersData():
	with open('data/Pitching.csv', 'rb') as csvfile:
		reader = csv.reader(csvfile)
		data = []
		reader.next()
		for row in reader:
...
def loadPlayerInfo():
	with open('data/Master.csv', 'rb') as csvfile:
		reader = csv.reader(csvfile)
		data = {}
		reader.next()
		for row in reader:
...
def getTeamData():
	...
	for year, values in leagueAdjust.iteritems():
...
def combineYearsByKey(years):
	...
	for key, value in runningYears.iteritems():
